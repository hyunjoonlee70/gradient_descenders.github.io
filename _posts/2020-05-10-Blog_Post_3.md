...
The second limitation (which is also the crux of our project) is addressed in a similar fashion, by appending additional tokens as \`[coarse conditioning]^[fine conditioning] (\` as the delimiter, ^ denoting superscript) to each instance of the dataset.
